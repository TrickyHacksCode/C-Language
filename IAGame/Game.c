/* IntroduÁ„o ‡ ProgramaÁ„o - Projeto de 2011

   Game.c
   Artur Miguel Dias, 2011
   
   O C„o Dancarino contra os Carangejos Mutantes Assassinos
   ...
   ‡ vontade, a comeÁar neste coment·rio.
*/
...
/* ATORES - CONSTANTES, TIPOS E VARI¡VEIS */
...
/* MUNDO - CONSTANTES, TIPOS E VARI¡VEIS */
...
void fazer_movimento_perseguidor(Ator p, int dx, int dy)
{
	...
	{
		// Em vez de ficarem parados, vao se movimentando ate terem hipotese de ir em direcÁao ao heroi.
		switch(random){
			case 0:
			//cima
			dx=0; dy=-1;
...
void perseguidor_movimentacao(Ator p)
{
	int dx = 0;
	int dy = 0;
// FAZER: O perseguidor toma uma decis„o de movimentaÁ„o e move-se.
// Para comeÁar os perseguidores andar sÛ para a direita.
// Depois poder· tornar os perseguidores um pouco mais inteligentes.
	if(heroi->x < p->x)
	{
		if( heroi->y > p->y)
		{
...
{
// FAZER: criar as defesas em posiÁıes aleatÛrias livres
int i,x,y;
...
{
// FAZER: cria os perseguidores em posiÁıes aleatÛrias livres
int i,x,y;
...
{	
	...
	if(file == NULL) {
		//não existe ficheiro de jogo
		//mensagem de erro para stderr
		fprintf(stderr,"ERRO: Não existe ficheiro de jogo!");
...
{
    Alert("About",
             "O C„o DanÁarino defende-se dos Carangejos Mutantes Assassinos\n\n"
             "por Otelo e DesdÈmona\n\nFCT/UNL, 2011");
}
